...
RSpec.describe "リスト登録機能", type: :request do
  ...
  let(:cafe) { create(:cafe, user: other_user) }
  ...
  context "リスト登録/解除機能" do
    context "ログインしている場合" do
      ...
      it "投稿のリスト登録/解除ができること" do
        expect {
          post "/lists/#{cafe.id}/create"
      ...
      it "投稿のAjaxによるリスト登録/解除ができること" do
        expect {
          post "/lists/#{cafe.id}/create", xhr: true
        }.to change(other_user.lists, :count).by(1)
        expect {
